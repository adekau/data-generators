...
describe('Data Generators: Map', () => {
    describe('pipe', () => {
        it('should pipe map', () => {
            ...
            expect(gen.create()).toEqual(expect.any(String));
        });
        ...
        it('should pipe flatMap', () => {
            ...
            expect(result.every((s) => s.length <= 10 && s.length >= 1)).toBe(true);
        });
        ...
        it('should pipe 9 functions', () => {
            ...
            expect(gen.createMany(5).every((x) => (typeof x === 'string' ? x.length === 1 : x >= 1 && x <= 10))).toBe(
                true
            );
        });
    });
